...
/* Load an FMSQ slot by reading its bytes from LittleFS (host-side flash/
 * directory on Linux), then storing them in the same music_tracks[] table
 * that LOAD_BINARY uses. Lets us bypass the inline IPC payload limit
 * (~150 B) for larger BGM files. */
static int process_load_fmsq_file_command(const fmrb_audio_load_fmsq_file_cmd_t *cmd, size_t total_size) {
    ...
    ESP_LOGI(TAG, "Load FMSQ file: music_id=%u path=%s", cmd->music_id, full_path);
    ...
    ESP_LOGI(TAG, "Loaded FMSQ slot %u from %s (%ld bytes)", cmd->music_id, full_path, sz);
    return 0;
}
